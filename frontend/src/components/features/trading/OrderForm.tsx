...
export const OrderForm = ({universalCode}: OrderFormProps) => {
    const shortCode = universalCode?.split(':')[1] ?? '—';
        ...
        <Tabs defaultValue="buy" className="w-full h-full flex flex-col">
            ...
            <TabsContent value="buy" className="m-0 p-4 space-y-4 flex-1 flex flex-col">
                <div className="space-y-3 flex-1">
                    ...
                    <div className="space-y-1">
                        <div className="text-[10px] text-slate-500 font-bold uppercase tracking-wider">주문수량
                            ({shortCode})
                        </div>
                        <Input
                            type="text"
                    ...
                    <div className="grid grid-cols-4 gap-1">
                        {['10%', '25%', '50%', '100%'].map((p) => (
                            <Button key={p} variant="outline"
                                    className="h-7 text-[10px] bg-slate-800 border-slate-700 text-slate-300 hover:bg-slate-700">
                                {p}
                            </Button>
                        ))}
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <span className="text-lg font-mono font-bold text-red-500">{total} <span
                            className="text-xs">KRW</span></span>
                    </div>
                </div>
                <Button
                    className="w-full h-12 bg-red-600 hover:bg-red-700 font-bold text-md shadow-lg shadow-red-900/20">
                    매수하기
                </Button>
            </TabsContent>
