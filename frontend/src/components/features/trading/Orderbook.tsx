    ...
    return (
        <ScrollArea className="flex-1 p-0">
            <div className="flex flex-col text-[11px] font-mono leading-none">
                ...
                {sells.map((sell, i) => (
                    <div key={`sell-${i}`}
                         className="grid grid-cols-2 h-[34px] group border-b border-slate-800/10 cursor-pointer relative overflow-hidden">
                        <div
                            className="flex items-center justify-end px-2 bg-blue-500/[0.03] text-blue-400 border-r border-slate-800/50">
                            {sell.price.toLocaleString()}
                        </div>
                        <div className="flex items-center justify-start px-2 bg-slate-950/30 text-slate-400 relative">
                            <div className="absolute left-0 top-0 bottom-0 bg-blue-500/10 transition-all duration-300"
                                 style={{width: `${sell.ratio}%`}}/>
                ...
                {buys.map((buy, i) => (
                    <div key={`buy-${i}`}
                         className="grid grid-cols-2 h-[34px] group border-b border-slate-800/10 cursor-pointer relative overflow-hidden">
                        <div
                            className="flex items-center justify-end px-2 bg-red-500/[0.03] text-red-400 border-r border-slate-800/50">
                            {buy.price.toLocaleString()}
                        </div>
                        <div className="flex items-center justify-start px-2 bg-slate-950/30 text-slate-400 relative">
                            <div className="absolute left-0 top-0 bottom-0 bg-red-500/10 transition-all duration-300"
                                 style={{width: `${buy.ratio}%`}}/>
